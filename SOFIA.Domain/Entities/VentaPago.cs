...
public sealed class VentaPago : BaseEntity
{
    ...
    public static Result<VentaPago> Create(
        ...
        transaccionId == Guid.Empty
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.TransaccionId", "Transacción ID is required."))
            : string.IsNullOrWhiteSpace(metodoPago)
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.MetodoPago", "Método de Pago is required."))
            : metodoPago.Length > 50
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.MetodoPago", "Método de Pago must not exceed 50 characters."))
            : montoPagado <= 0.0m
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.MontoPagado", "Monto Pagado must be greater than 0."))
            : referenciaOperacion != null && referenciaOperacion.Length > 100
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.ReferenciaOperacion", "Referencia de Operación must not exceed 100 characters."))
            : fechaPago == default
            ? Result.Failure<VentaPago>(Error.Validation("VentaPago.FechaPago", "Fecha de Pago is required."))
            : Result.Success(new VentaPago
            {
