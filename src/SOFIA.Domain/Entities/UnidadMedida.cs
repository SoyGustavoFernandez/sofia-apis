...
public sealed class UnidadMedida : BaseEntity
{
    ...
    public static Result<UnidadMedida> Create(string codigo, string descripcion)
    {
        ...
        {
            return Result.Failure<UnidadMedida>(Error.Validation("UnidadMedida.Codigo", "Codigo is required."));
        }

        if (codigo.Length > 10)
        {
            return Result.Failure<UnidadMedida>(Error.Validation("UnidadMedida.Codigo", "Codigo must not exceed 10 characters."));
        }

        if (string.IsNullOrWhiteSpace(descripcion))
        {
            return Result.Failure<UnidadMedida>(Error.Validation("UnidadMedida.Descripcion", "Descripcion is required."));
        }

        if (descripcion.Length > 50)
        {
            return Result.Failure<UnidadMedida>(Error.Validation("UnidadMedida.Descripcion", "Descripcion must not exceed 50 characters."));
        }
