...
public sealed class PacienteCliente : BaseEntity
{
        ...
        string? contactoPrimario)
    {
        ...
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.DocIdentidadGub", "Documento de Identidad Gubernamental is required."));
        }

        if (docIdentidadGub.Length > 50)
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.DocIdentidadGub", "Documento de Identidad must not exceed 50 characters."));
        }

        if (string.IsNullOrWhiteSpace(nombreApellidos))
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.NombreApellidos", "Nombre y Apellidos is required."));
        }

        if (nombreApellidos.Length > 200)
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.NombreApellidos", "Nombre y Apellidos must not exceed 200 characters."));
        }

        if (fechaNacimiento == default)
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.FechaNacimiento", "Fecha de Nacimiento is required."));
        }

        if (fechaNacimiento > DateOnly.FromDateTime(DateTime.UtcNow))
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.FechaNacimiento", "Fecha de Nacimiento cannot be in the future."));
        }

        if (contactoPrimario != null && contactoPrimario.Length > 100)
        {
            return Result.Failure<PacienteCliente>(Error.Validation("PacienteCliente.ContactoPrimario", "Contacto Primario must not exceed 100 characters."));
        }

        return Result.Success(new PacienteCliente
        {
