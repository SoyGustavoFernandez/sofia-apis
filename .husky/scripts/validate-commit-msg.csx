...
var pattern = @"^(feat|fix|docs|style|refactor|perf|test|chore)\((domain|app|infra|api|ia|db|test)\): .{1,}$";
...
if (!Regex.IsMatch(commitMsg, pattern))
{
    ...
    Console.WriteLine("Alcances válidos: domain, app, infra, api, ia, db, test");
