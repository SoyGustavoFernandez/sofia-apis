...
public sealed class DespachoDelivery : BaseEntity
{
    ...
    public static Result<DespachoDelivery> Create(
        ...
        ventaId == Guid.Empty
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.VentaId", "Venta ID is required."))
            : string.IsNullOrWhiteSpace(plataformaServicio)
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.PlataformaServicio", "Plataforma de servicio is required."))
            : plataformaServicio.Length > 50
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.PlataformaServicio", "Plataforma de servicio must not exceed 50 characters."))
            : string.IsNullOrWhiteSpace(direccionEntrega)
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.DireccionEntrega", "Dirección de entrega is required."))
            : direccionEntrega.Length > 255
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.DireccionEntrega", "Dirección de entrega must not exceed 255 characters."))
            : codigoRastreo != null && codigoRastreo.Length > 100
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.CodigoRastreo", "Código de rastreo must not exceed 100 characters."))
            : repartidorNombre != null && repartidorNombre.Length > 150
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.RepartidorNombre", "Nombre de repartidor must not exceed 150 characters."))
            : evidenciaFotograficaUrl != null && evidenciaFotograficaUrl.Length > 500
            ? Result.Failure<DespachoDelivery>(Error.Validation("DespachoDelivery.EvidenciaFotograficaUrl", "Evidencia fotográfica URL must not exceed 500 characters."))
            : Result.Success(new DespachoDelivery
            {
                VentaId = ventaId,
