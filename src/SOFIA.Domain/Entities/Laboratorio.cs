...
public sealed class Laboratorio : BaseEntity
{
    ...
    public static Result<Laboratorio> Create(string nombreCompania, string? codigoIdentificador)
    {
        ...
        {
            return Result.Failure<Laboratorio>(Error.Validation("Laboratorio.NombreCompania", "Nombre de Compañía must not exceed 150 characters."));
        }

        if (codigoIdentificador != null && codigoIdentificador.Length > 50)
        {
            return Result.Failure<Laboratorio>(Error.Validation("Laboratorio.CodigoIdentificador", "Código Identificador must not exceed 50 characters."));
        }

        return Result.Success(new Laboratorio
        {
