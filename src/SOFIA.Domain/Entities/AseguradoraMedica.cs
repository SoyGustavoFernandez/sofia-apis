...
public sealed class AseguradoraMedica : BaseEntity
{
        ...
        string codigoIdentificadorNacional)
    {
        ...
        {
            return Result.Failure<AseguradoraMedica>(Error.Validation("AseguradoraMedica.NombreComercial", "Nombre comercial is required."));
        }

        if (nombreComercial.Length > 150)
        {
            return Result.Failure<AseguradoraMedica>(Error.Validation("AseguradoraMedica.NombreComercial", "Nombre comercial must not exceed 150 characters."));
        }

        if (string.IsNullOrWhiteSpace(codigoIdentificadorNacional))
        {
            return Result.Failure<AseguradoraMedica>(Error.Validation("AseguradoraMedica.CodigoIdentificadorNacional", "Código identificador nacional is required."));
        }

        if (codigoIdentificadorNacional.Length > 50)
        {
            return Result.Failure<AseguradoraMedica>(Error.Validation("AseguradoraMedica.CodigoIdentificadorNacional", "Código identificador nacional must not exceed 50 characters."));
        }

        return Result.Success(new AseguradoraMedica
        {
