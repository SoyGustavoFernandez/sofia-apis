...
public sealed class ProfesionalSalud : BaseEntity
{
    ...
    public static Result<ProfesionalSalud> Create(
        ...
        string.IsNullOrWhiteSpace(numeroRegistro)
            ? Result.Failure<ProfesionalSalud>(Error.Validation("ProfesionalSalud.NumeroRegistro", "Número de Registro is required."))
            : numeroRegistro.Length > 50
            ? Result.Failure<ProfesionalSalud>(Error.Validation("ProfesionalSalud.NumeroRegistro", "Número de Registro must not exceed 50 characters."))
            : string.IsNullOrWhiteSpace(nombrePrescriptor)
            ? Result.Failure<ProfesionalSalud>(Error.Validation("ProfesionalSalud.NombrePrescriptor", "Nombre de Prescriptor is required."))
            : nombrePrescriptor.Length > 150
            ? Result.Failure<ProfesionalSalud>(Error.Validation("ProfesionalSalud.NombrePrescriptor", "Nombre de Prescriptor must not exceed 150 characters."))
            : direccionClinica != null && direccionClinica.Length > 255
            ? Result.Failure<ProfesionalSalud>(Error.Validation("ProfesionalSalud.DireccionClinica", "Dirección de la Clínica must not exceed 255 characters."))
            : Result.Success(new ProfesionalSalud
            {
